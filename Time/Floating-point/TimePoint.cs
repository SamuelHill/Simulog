...
namespace Time {
    ...
    public readonly struct TimePoint : IComparable<TimePoint>, IEquatable<TimePoint>, ISerializableValue<TimePoint> {
        ...
        public TimePoint(float clock) => Clock = clock;

        // ReSharper disable once InconsistentNaming MemberCanBePrivate.Global
        /// <summary> The last possible TimePoint (in this case float.NaN) </summary>
        public static readonly TimePoint Eschaton = new(float.NaN);
        private bool IsEschaton => float.IsNaN(Clock);
        ...
        /// <inheritdoc />
        public bool Equals(TimePoint other) => Math.Abs(Clock - other.Clock) < Tolerance;
        /// <inheritdoc />
        ...
        public static bool operator ==(TimePoint t1, float t2) => Math.Abs(t1.Clock - t2) < Tolerance;
        ...
        public static bool operator >(TimePoint t1, float t2) => t1.Clock.CompareTo(t2) == 1;
        ...
        public static TimePoint operator +(TimePoint t1, float t2) => new(t1.Clock + t2);
        ...
        /// <summary> <inheritdoc cref="TimePoint.op_Addition(TimePoint,float)" path="summary"/> </summary>
        public static Function<TimePoint, float, TimePoint> AddToTimePoint = new(nameof(AddToTimePoint), AddToTimePointImplementation, false)
            {NameForCompilation = nameof(AddToTimePointImplementation)};
        
        /// <summary> <inheritdoc cref="TimePoint.op_Addition(TimePoint,float)" path="summary"/> </summary>
        /// <param name="x"><inheritdoc cref="TimePoint.op_Addition(TimePoint,float)" path="param[@name='t1']/node()"/></param>
        ...
        // <param name="y"><inheritdoc cref="TimePoint.op_Addition(TimePoint,float)" path="param[@name='t2']/node()"/></param>
        public static TimePoint AddToTimePointImplementation(TimePoint x, float y) => x + y;

        /// <returns>TimePoint as an floating point string with the text "Has not occurred" for Eschaton TimePoints.</returns>
        public override string ToString() => IsEschaton ? "Has not occurred" : Clock.ToString(CultureInfo.InvariantCulture);
        ...
        public static TimePoint FromString(string timePointString) {
            ...
            if (float.TryParse(timePointString, out var i)) return new TimePoint(i);
